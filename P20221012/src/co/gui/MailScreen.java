...
public class MailScreen extends JFrame {
	...
	JPanel center, bottom; // 컴포넌트를 모아두기 위한 컨테이너 만들기
	JLabel fromLbl, toLbl, subjectLbl, contentLbl; // 레이블
	JTextField fromTxt, toTxt, subjectTxt; // 텍스트필드
	JTextArea contentTxt; // 여러라인에 걸쳐서 작업해야해서 area에 적기
	JButton send, cancel;
	...
	public MailScreen() {
		...
		contentTxt = new JTextArea(10,25); // 행,열 값
		
		// button
		send = new JButton("보내기");
		send.addActionListener(new MyActionListener()); // ActionListener은 인터페이스 -> 구현객체
